...
if __name__ == '__main__':
    ...
    if args.input_image is not None:
        ...
        mask_out_path = os.path.join(args.output_dir, "mask_" + '.'.join(img_name.split('.')[:-1]) + '.png')
        print(mask_out_path)

        inference_image(model, device, args.input_image, args.prompt_image, args.prompt_target, out_path, mask_out_path)
