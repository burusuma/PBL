...
def inference_image(model, device, img_path, img2_paths, tgt2_paths, out_path, mask_out_path=None):
    res, hres = 448, 448
...
def inference_video(model, device, vid_path, num_frames, img2_paths, tgt2_paths, out_path):
    ...
    if img2_paths is None:
        ...
        img2 = Image.open(img2_paths).convert("RGB")
    img2 = img2.resize((res, hres))
    img2 = np.array(img2) / 255.

    tgt2 = Image.open(tgt2_paths).convert("RGB")
    tgt2 = tgt2.resize((res, hres), Image.NEAREST)
    tgt2 = np.array(tgt2) / 255.
    ...
    while True:
        ...
        output = input_image * (0.9 * output / 255 + 0.4)
